...
A [k9s](https://k9scli.io/)-inspired TUI for Docker, built with [Textual](https://textual.textualize.io/) and the [Docker SDK for Python](https://docker-py.readthedocs.io/).
...
The main view. Shows all Docker containers with ID, name, image, state, ports, age, and compose project path (read from the `com.docker.compose.project.working_dir` label — shows `-` for non-compose containers).
...
Docker images with ID, repository, tag, size, and creation date. Inspect details (tags, digests, architecture, layers, labels), delete individual images, or prune all dangling ones.

### 3 — Volumes

Docker volumes with name, driver, and mountpoint. Inspect details (driver, mountpoint, options, labels), delete individual volumes, or prune all unused ones.

### 4 — Networks

Docker networks with ID, name, driver, scope, subnet, and connected container count. Inspect details (IPAM config, connected containers, labels), delete individual networks, or prune unused ones.

### 5 — Compose

Lists all Docker Compose projects detected from container labels (`com.docker.compose.project`, `com.docker.compose.service`, etc.). Shows project name, working directory, service count, and running/stopped counts.
...
If you do `docker compose down`, containers are removed and the project disappears from this view. Use `:up` or `W` from the Containers view to bring them back.
...
Press `y` to toggle **raw JSON** — the full `docker inspect` output from the Docker API.
...
| `s` | Shell into container (`docker exec -it`) |
| `S` | Live stats (CPU, memory, network I/O, block I/O, PIDs) |
| `ctrl+d` | Remove container — `docker rm -f` (`x` toggles volume removal in confirm dialog) |
| `ctrl+k` | Kill container (SIGKILL) |
| `w` | Compose down (`x` toggles `-v` for volume removal) |
| `W` | Compose up — restarts the last project you downed, or the selected container's project |
| `P` | System prune — `sudo docker system prune --all` (prompts for password, `x` toggles `--force`) |
...
Press `:` to open. Tab autocompletes command names. For `:up <path>`, Tab cycles through matching directories/files.
...
| `:up` | Fuzzy file finder — scans for compose files, type to filter, Enter to select and `docker compose up -d` |
| `:up <path>` | Compose up from a specific file or directory (supports tab path completion) |
...
- Python 3.10+
- Docker daemon running (local socket or `DOCKER_HOST` env)
- `textual >= 8.0`
- `docker >= 7.0`
